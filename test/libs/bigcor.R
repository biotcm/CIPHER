# Calculate correlations for big matrices
#
# References:
# - https://rmazing.wordpress.com/2013/02/22/bigcor-large-correlation-matrices-in-r/
# - http://www.dr-spiess.de/scripts/bigcor.R
...
bigcor <- function(
  ...
  fun <- match.arg(fun)
  FUN <- ifelse(fun == "cor", cor, cov)
  STR <- ifelse(fun == "cor", "Correlation", "Covariance")
  ...
  if (is.null(y)) {
    ...
    resMAT <- matrix(nrow = NCOL, ncol = NCOL)
  } else {
    # resMAT <- ff(vmode = "double", dim = c(NCOL, YCOL))
    resMAT <- matrix(nrow = NCOL, ncol = YCOL)
  }
