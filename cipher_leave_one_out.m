...
tic
pheno_gene_relation = cell(phenoNum, 1);
...
tic
result_resolution = 10000;

test_result = [];
for phenoIndex = 1:phenoNum
    if isempty(pheno_gene_relation{phenoIndex})
        continue
    end
    fprintf('Validating the %dth phenotype...\n',phenoIndex);
    origin_array = pheno_gene_relation{phenoIndex};
    backup_closeness = gene2phenotype_closeness(:, phenoIndex);
    % For each target
    if length(origin_array) == 1
        ...
        for dgIndex = 1:length(origin_array) % Disease gene index
            array = origin_array([1:(dgIndex-1), (dgIndex+1):end]);
            % Recalculate gene2phenotype closeness
            for geneIndex = 1:geneNum
                gene2phenotype_closeness(geneIndex, phenoIndex) = sum(exp(-(Protein_Shortest_Distance(geneIndex,array)).^2));
            end
    ...
    gene2phenotype_closeness(:, phenoIndex) = backup_closeness;
end
test_result = test_result / result_resolution;

toc
clear phenoIndex dgIndex backup_closeness origin_array
%% Show result
figure
% subplot(2,1,1)
cdfplot(test_result)
% subplot(2,1,2)
% hist(test_result, result_resolution)
